<div align="center">

# 🎬 Movie Info Simplified
*A demo Django-based app exemplifying DevOps best practices.*

> **🚧 Work in Progress**  
> This is an ongoing portfolio in **constant development**.  
> ⭐ **Love it?** Leave a star!  
> 🐛 **Found issues?** Open a discussion!

[![GitHub stars](https://img.shields.io/github/stars/benabuariel/movie-info-simplified)](https://github.com/benabuariel/movie-info-simplified)
[![GitHub issues](https://img.shields.io/github/issues/benabuariel/movie-info-simplified)](https://github.com/benabuariel/movie-info-simplified/issues)
[![License: MIT](https://img.shields.io/badge/License-MIT-yellow.svg)](LICENSE)

[![Docker Compose](https://img.shields.io/badge/Docker-Compose-blue?logo=docker)](docker-compose.yml)
[![Django](https://img.shields.io/badge/Django-5.0-green?logo=django)](https://www.djangoproject.com/)
[![Gunicorn](https://img.shields.io/badge/Gunicorn-22.0-yellow)](https://gunicorn.org/)
</div>

## 🚀 Quick Start

**With Docker Compose (recommended):**
```bash
docker compose up -d --build     # Start stack
docker compose logs -f web       # View logs
docker compose down -v           # Clean stop
```
## 🎯 Purpose
I built this project to prove I have practical experience with DevOps tools. It demonstrates containerization, CI/CD pipelines, cloud infrastructure, and GitOps workflows using industry-standard practices.

## ✨ Features
- Django-powered movie information app
- Responsive UI for browsing movie details
- [TBD: Add more app features here]
- Dockerized for easy local development and deployment
- [TBD: Live demo link after deployment]

## 🛠 Tech Stack
- Docker
- Docker-compose
- Kubernetes
- AWS
- CI/CD using Jenkins and Groovie

## CI/CD (Planned)

- On each push: run tests and build Docker image.
- On main branch: push image to container registry.
- Future: deploy container to AWS (ECS/EKS) using GitOps tooling.